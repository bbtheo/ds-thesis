...
Master's thesis project: **Test-Time Context Construction for Tabular Foundation Models under Heavy Class Imbalance** (subtitle: "Retrieval and class-ratio control in financial fraud detection"). Retitled 2026-07-26 from "Retrieval-Augmented Prediction (RAP) for ..." because RAP turned out to be the falsified hypothesis, not the contribution; the abstract was rewritten the same day to state both verdicts.

The thesis evaluates test-time context construction strategies for ICL-based tabular foundation models (TFMs; the chapters use "TFM", not "FTM"). Instead of random subsampling the training context (which under heavy class imbalance floods the model with irrelevant legitimate transactions), RAP retrieves a structurally similar, label-balanced context around each test batch using kNN retrieval and controlled fraud ratio oversampling. RAP is a negative result; stratified ratio control (C2) is the positive finding.
...
This file is the authoritative record of the experiment design. The original planning document (`scratchpad/experiment-plan.md`) was removed with the rest of `scratchpad/` on 2026-07-14 (purged from git history, local and remote); the result narrative and RQ mapping now live in `thesis-outline.md`.
...
- **CatBoost runs with `thread_count=1`** (set in `gbdt.py`). At the default thread count CatBoost 1.2.10 SIGSEGVs in its TBB pool during the long fit on paysim (~5M rows) and took down the whole unattended driver on 2026-06-14. It is slower on the largest datasets and **not fully result-neutral (corrected 2026-09-19):** 1 vs 24 threads are bit-identical on eu_cc (no categoricals) but differ slightly on fifar/baf (cause not isolated; banksim, with more categoricals, reproduces exactly): |ΔPR-AUC| ≤ 1e-3, |Δrecall@FPR| ≤ 0.0063 at seed 42 (PR-AUC inside the 0.005 tie band, the worst recall delta just above it). The grid catboost rows for every dataset except paysim were fitted at 24 threads before the pin (each thread count is deterministic on its own), so a `thread_count=1` refit of fifar/baf catboost does not reproduce those rows exactly; `results/preds/README.md` documents this (the confidence appendix that used these runs was removed from the thesis on 2026-09-20; the readout study replaces it as the row-level view). NOT a memory/data/metric issue (load is 3.5 GB/4 s). See the auto-memory note `catboost-paysim-segfault`.
- **C3/C4 (RAP retrieval) are implemented (Phase 3, 2026-06-18).** `src/rap/{retriever,sampler,context}.py` plus a per-group FTM path in the runner. Per-group design (one retrieval + one FTM refit per silhouette-chosen test cluster); no `pipeline_version` bump (C1/C2/GBDT hashes/results untouched, only new C3/C4 rows). C3/C4 FTMs are `tabpfn_3` + `tabiclv2` only. Smoke-tested on `ai_banking` (tabiclv2 C3+C4) and `eu_cc` (tabpfn_3 C3) — idempotent, finite metrics. Dev inspection: `scripts/dev_rap_validation.py`. **Phase-4 grid driver built + launched (2026-06-18):** `scripts/run_pending_c3c4.py` enumerates every pending C3/C4 formal-grid cell (cc_2025 excluded): C3 + C4 ratio sweep {0.05,0.10,0.20,0.30,0.50} × {`tabiclv2`,`tabpfn_3`} × 13 dataset-seed units = **156 runs** (`metric=cosine`, `n_estimators=4`); idempotent, loads each split once. **All grids are COMPLETE** (formal C1–C4, the C2 ratio × context-size grid, the RAP design-space sweep, and the relevance ablation). Headline: **RAP retrieval is a negative result** (C3 < C1 and C4 < C2 on all 5 informative datasets); **C2 stratified balancing is the positive finding** (zero-shot FTMs at/above external SOTA). See `thesis-outline.md` for the full result narrative with provenance.
- **Full review applied (2026-07-06):** all corrections from the 2026-07-06 review are implemented (the correction log lived in `scratchpad/`, since removed) — outline/bib fixes, `ap_at_prevalence` tie fix (+ `tests/test_metrics.py`), thread-pinned C3/C4 grouping (`threadpoolctl`), per-group retrieval now timed into `setup_s`, runner validation (C3/C4 model restriction, stray `fraud_ratio` rejected), verified checkpoint context limits (v3 = 1M, 2.6 = 100k — our 50k is a design budget), stale parquets quarantined. BLK-1 resolved: the eu_cc SOTA figure 0.897 was the single best run (tabpfn_3, C2, seed 7); the fair seed-mean is **0.869** vs external 0.867 → "at par", not "above".
- **Repo cleaned for public release (2026-07-07):** deleted the abandoned-direction lit files, one-off probe scripts, superseded drivers (`run_c2.py`, `diag_c1_vs_c4.py`, `diag_rap_designspace.py`, the `*_daemon.sh` wrappers), rendered artifacts (`datasets.html`, `data-doc.typ`, tracked `.quarto/` cache), the dead `references.bib`, `main.py`, and the broken renv machinery (`.Rprofile`, `renv.lock`, `renv/`). `methodology.qmd` deleted (superseded draft). `results/runs_stale_prev4/` trimmed to the 23 cc_2025 files and renamed `results/runs_cc2025_degenerate/`; `results/diag/` trimmed to the two anchor-injection evidence files. **The EDA figures were deleted (user decision)** — the EDA figures cited in `thesis-outline.md` no longer exist in the repo and must be regenerated for the thesis document itself.
- **External SOTA comparison (2026-07-25):** external comparators were verified 2026-07-24 via a multi-agent search + adversarial verification; the compilation file `sota-sources.md` was removed with `thesis-outline.md` in commit 87e9f1f (chapter prose is now the record — per-source caveats live in the `note` fields of the `thesis-refs.bib` entries and in §6.5). The old scratchpad comparators banksim 0.896 / paysim 0.854 did NOT survive verification — verified figures are banksim 0.772 PR-AUC and no threshold-free paysim figure at all (F1 0.82 only → "not comparable"). Chapter section `sections/6_results.qmd` §6.5 (`sota-setup`/`tbl-sota-comparison`/`fig-sota-eucc`) computes all our-side values inline: 10-seed C2 means from `results/runs`, plus the eu_cc best-effort study in **`results/runs_sota_cv`** (`scripts/run_eucc_sota_cv.py`: stratified 5-fold CV over all 284,807 rows, full ~228k training fold as context, `n_estimators=12` — NEVER mix into grid pivots). Reporting decision: pooled predictions + stratified bootstrap CI (seed 42) for the best case (tabpfn_3 natural: AP 0.878, CI 0.851–0.903, verdict "at par" with external 0.871); per-fold means elsewhere (2 % duplication arm 0.871, does not help). **§6.5 reports ONLY full-context figures where they exist (Theo, 2026-07-25): eu_cc shows the CV protocol alone, never alongside the 50k grid cell (one entry per model per dataset — "no one cares about the formal grid" in the external comparison); other datasets show their C2 values flagged as context-limited/conservative.** **tabiclv2 CV run COMPLETE (2026-07-25):** natural pooled AP 0.879 (CI 0.851–0.904), fold-mean 0.880 ± 0.025, 2 % arm 0.869 — nominally ahead of tabpfn_3 (pooled 0.878) inside the tie band; verdict still "at par" (both CIs contain 0.871). **48k "pre-training bound" myth corrected 2026-07-25 against the TabICLv2 paper (arXiv:2602.11139):** the pretraining curriculum tops out at 60k-row datasets (48k appears nowhere in the paper — it is only our experimental budget in `ftm.py`), and the paper validates inference on tables up to 600k rows / million-scale, so the ~229k full-fold context is in-scope, NOT extrapolation; §6.5 prose states this. Render-failing guards in `sota-setup` assert the banksim "above" and eu_cc "at par" verdicts.
...
├── sections/                  # thesis chapter sources included by thesis.qmd (8 chapters + X_appendix.qmd)
├── thesis.qmd                 # main thesis document (Quarto → Typst PDF / HTML; CI renders on push)
├── thesis-outline.md          # concise 8-chapter narrative outline with verdicts + open-gaps audit
...
## Decoder-readout case study (2026-09-20)

`scripts/run_readout.py` → `results/readout/` (see its README). Supervisor-requested
context-importance visualisation (PriorLabs decoder readout, `tabpfn_extensions.
interpretability.get_decoder_readout`); feeds §5.1 of the restructured thesis. Ran on
this machine in a **separate venv** `.venv-readout` (tabpfn **8.5.0**, tabpfn-extensions
0.6.2, torch 2.14.0+cu130; gitignored, created with `uv venv` + `uv pip install`, the
main `uv.lock` is untouched) because the readout needs `tabpfn>=8.4` and exists only for
the v3 architecture. `TabPFNModel` gained `readout()` (raises under 8.0.8) and keeps
`X_ctx_`/`y_ctx_`. Cells: eu_cc + banksim × {C1, C2 at the 10 % plateau ratio (C2-grid
sampler, NOT the formal 50/50 C2), C4 r=0.10 cosine} × tabpfn_3 × seed 42 × n_est=4;
contexts are byte-identical to the corresponding grid cells (same code + RNG streams,
C4 grouping reproduced G=9/G=2). Readout subset = all scored test frauds + 2,000 seeded
scored legits; full scored set predicted for the drift check: |ΔPR-AUC| ≤ 0.0027 vs the
grid (tie band 0.005), Recall@5%FPR identical in 5/6 cells. **These files are a different
library version and must never be pooled with `results/runs*`.** Finding: under C4 fraud
test rows attend MORE to fraud context than under C2; the banksim failure is legit test
rows placing ~0.52 of their mass on retrieved fraud context (eu_cc legit rows: 0.009).
`scripts/readout_support.py` adds `nn_coverage.parquet` (per-row cosine distance to nearest legit/fraud context row under C2/C4) and `example_rows.parquet` (banksim worked example, decoded categoricals). Mechanism seen in the data: C4 retrieves all frauds but only a thin legit slab, so 64 % of banksim legit test rows have a fraud context row closer than any legit one (C2: 2 %); the false positives hurt both PR-AUC and Recall@FPR. No write-up exists in the repo (an `interpret.qmd` was drafted and deleted the same day, Theo's decision); §5.1 thesis figures still to be written. Total wall 52 min.
