...
- **Not created yet:** the `analysis/` R scripts (Phase 5). The run scripts in `scripts/` serve as the grid driver. `thesis.qmd` now includes all nine chapter files in `sections/` plus `X_appendix.qmd` and renders end-to-end in both formats (HTML + Typst PDF, verified 2026-07-25); chapters carry their own R chunks, so `thesis-outline.md` is the narrative record, not the render source.
