...
- **CI seed extension (launched 2026-07-18, runs in progress):** the two SOTA FTMs (tabpfn_3, tabiclv2) × {C1, C2} × all 5 formal datasets extended from 3 seeds (fifar 1) to a fixed **a-priori** 10-seed list ({0–6, 42, 123, 7}, identical for every dataset; all seeds reported, none dropped), n_estimators=4 through the unchanged v4 pipeline (148 new runs). Purpose: per-dataset bootstrap CIs over repetitions for the C2/SOTA values (§6.11) and seed-paired C2−C1 deltas at n=10; extending only the headline cells is conservative (a lucky-seed mean can only regress — the eu_cc 0.897→0.869 lesson institutionalized). fifar's fixed split means its seeds vary context sampling only [finding: scripts/run_seed_extension.py; results/runs]
