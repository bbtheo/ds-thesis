...
> **Central result — honest reframing:** RAP's retrieval-relevance lever is a **negative result** — C4 (RAP) < C2 (stratified) on every dataset and C3 (kNN) < C1 (random) on all but baf (a tie: +0.005/+0.006, within the ±0.005 band at the separability floor), with per-dataset attributed causes (lost negative diversity where retrieval collapses; duplication degeneracy on eu_cc — §7.2). The *positive*, salvageable finding is that **stratified context balancing (C2) is the best context-construction strategy and is competitive with sound external SOTA at no per-dataset tuning** (the effective optimum is a broad plateau near r≈0.10, not literal 50/50). The thesis is an honest relevance-vs-ratio decomposition, not a "RAP wins" story.
>
...
> - `[finding: results/runs_c2grid]` = C2 fraud-ratio grid (1092 cells: 810 core + 144 large-context + 138 net-new n_estimators); `[finding: results/runs_rapds]` = RAP design-space sweep (720 cells); `[finding: results/diag]` = mechanism diagnostics.
...
- C3 (kNN, natural ratio) is WORSE than C1 on 4/5 datasets, sometimes catastrophic; matched-seed deltas (tabpfn_3/tabiclv2): paysim −0.78/−0.82, eu_cc −0.26/−0.47, banksim −0.04/−0.06, fifar −0.01/−0.02 (n=1 split) [finding: results/runs, C3 vs C1, n_est=4, matched seeds {7,42,123} — C1 for these two models carries 10 seeds, so unmatched means silently mix]
  - baf (+0.006/+0.005) is a TIE within the ±0.005 band at the separability floor, not a positive exception (one of six runs negative) [finding: results/runs]
  - per-seed spread is extreme and bimodal (worst cells: paysim tabpfn_3 seed42 = 0.0225; eu_cc tabiclv2 seed123 = 0.087) [finding: results/runs]
- the starvation-vs-composition decomposition is DATASET-DEPENDENT (verified 2026-07-21; "not primarily starvation" holds only on paysim): paysim ≈75 % legit-composition (fraud-rich seed-7 runs, ~820–832 frauds, still collapse to 0.21/0.30, at the KR composition floor) + ≈25 % starvation (4 runs at ~1 fraud → 0.02–0.06); eu_cc ≈90 % depletion/starvation (starved seeds: 6 frauds, G=2, realized ratio ~14× depleted → 0.09–0.51; fraud-adequate seed 42, 105–107 frauds, G=9 → 0.755/0.765 ≈ C1). Do NOT argue from the "~39 frauds/group on average" figure: it averages a bimodal {6, 105, 6} distribution [finding: results/runs, C3 context_fraud_n per seed; results/runs_ablation]
- the silhouette picks a COARSE grouping (G=2 on banksim/paysim/fifar, ~4 on eu_cc, ~10 on baf); the ratio-matched ablation (§5.3, §7.2) isolates the damage to the LEGIT selector: KR−RR ≈ −0.72 on paysim (tabpfn_3 −0.94), −0.03 banksim, −0.07 eu_cc; RK−RR within ±0.02; forcing finer G still loses to C2 everywhere (§6.7) [finding: results/runs, n_groups/group_silhouette; results/runs_ablation]
...
- the **natural-ratio ablation arms** (2026-07-18: {RR, KK} × ratio = train prevalence × G=32 × 50k, 3 datasets) give the granularity- and ratio-matched version of this contrast and carry the RQ3 verdict: kNN selection at natural ratio collapses paysim (RR 0.897 → KK 0.033, both models) and cuts banksim (0.901 → 0.785), but leaves eu_cc unchanged (0.834 → 0.845) — so eu_cc's main-grid C3 collapse (0.33/0.56) is carried by its drifted/depleted realized fraud ratio and the coarse silhouette grouping, not by locality itself [finding: results/runs_ablation, natural arms]
  - the eu_cc legit-selector sign is NOT robust: +0.011 in the natural both-flip contrast vs −0.075 in the enforced legit-only KR−RR (which adds duplication) → state eu_cc locality as neutral/sign-unstable, never "mildly positive" [finding: results/runs_ablation]
- per-dataset RQ3 verdict (mechanistic evidence covers 3/5 datasets; baf/fifar rest on the confounded main grid only): harmful on paysim (strong) and banksim (mild), neutral-once-controlled on eu_cc, tie at the floor on baf, directional only on fifar
...
- **RQ3 figure in the thesis (sections/6_results.qmd):** `fig-rq3-granularity` (G-recovery vs C1/C2 + legit_score panel). Three further RQ3 figures (per-seed C3-vs-C1 dot plot, starvation-decomposition scatter, ablation-arm dot plot) were drafted 2026-07-21 and cut by author decision the same day; the findings they showed stay in the §6.5 bullets as numbers
...
- mechanism evidence — `legit_score` (mean predicted score on legit rows; lower = better) falls as G refines: banksim 0.070→0.005, eu_cc ~0.02 mean (0.051 worst cell)→0.001 → coarse retrieval INFLATES legit scores (lost easy-legit anchors = §7.2-H1); paysim stays high (0.535→0.233). BUT (verified 2026-07-21) corr(legit_score, pr_auc): banksim −0.82, eu_cc −0.50, paysim **+0.15 (wrong sign)** → legit inflation is the mechanism on banksim and partly eu_cc only; it does NOT explain paysim, whose collapse is a distinct local-separability failure (do not call this a universal "smoking gun") [finding: results/runs_rapds, legit_score by n_groups + corr]
...
- [finding: scratchpad/eda/fig7_mechanism_panel.png — legit_score-vs-G panel + the banksim/tabiclv2 anchor-injection case study (created 2026-07-06)]; the G-recovery curve vs C1/C2 baselines now exists in the thesis as `fig-rq3-granularity` (sections/6_results.qmd, 2026-07-21), a 2×3 panel that also shows the paysim wrong-sign legit_score result
...
- observed ordering: C2 ≥ C1 on all 5 datasets; both retrieval conditions (C3, C4) fall below C1/C2 everywhere **except baf-C3** (0.168 vs C1 0.163 — a tie within the ±0.005 band, not a positive result); the C3-vs-C4 order is **dataset-dependent**, not a fixed C3>C4 — C4-best beats C3 on eu_cc (0.739 vs 0.555) and paysim (0.563 vs 0.128), while C3 beats C4 on baf/fifar; the full C2≥C1>C3>C4 chain holds only on banksim and fifar. Worst condition is dataset-specific: **C3** on paysim (0.128), C4 on baf/fifar — this **inverts** the draft's expected C1<C2≈C3<C4-best [finding: results/runs]
...
- **per-dataset attribution (not one universal principle; sharpened 2026-07-21):** on the 3-dataset ratio-matched ablation (eu_cc, banksim, paysim), the kNN *legit* selector is the **dominant driver where retrieval collapses** (KR−RR: paysim ≈−0.72, banksim ≈−0.03); on eu_cc the legit-selector effect is **sign-unstable** (+0.011 natural both-flip vs −0.075 enforced KR−RR; state as neutral, not positive) and the C3/C4 collapse there is instead carried by **duplication degeneracy** (H3) plus per-seed fraud depletion (§6.5); baf/fifar sit at a separability floor where neither mechanism moves much. The *lost-negative-diversity/legit-inflation* story is evidenced on banksim (corr(legit_score, pr_auc) −0.82) and partly eu_cc (−0.50) but NOT paysim (corr **+0.15**, wrong sign): paysim's legit-selector damage is real (KR−RR) but its mechanism is unidentified (§7.2 last bullet). Finer G recovers some ground (each tiny cluster's retrieval re-approaches a near-random balanced context, whose limit is C2 itself) but the recovery pattern also differs by dataset (§6.7) [finding: results/diag; results/runs_rapds; results/runs_ablation]
- **causal isolation (H1 — the RR-vs-KR contrast, both G=32; arm letters follow `scripts/run_relevance_ablation.py`: first = legit selector, second = fraud selector — an earlier outline draft had them transposed):** flipping only the *legit* selector from random to kNN, holding G=32 and ratio/context fixed, collapses paysim from ~0.97 (RR) to 0.49/0.03 (KR, tabiclv2/tabpfn_3); mild on banksim (−0.03); −0.07 on eu_cc (sign-unstable vs the natural contrast, per-dataset detail above). RR-vs-RK (the fraud-selector check) does almost nothing (within ±0.02). The granularity confound is **closed** (2026-07-18): the RR@G=32 arm matches both the global RR and RK@G=32 within seed noise on all 3 datasets (§5.3), so per-group granularity with random contexts is directly confirmed neutral and RK-vs-KK is a clean single-factor isolation of the legit selector. The natural-ratio arms corroborate the attribution on an independent contrast (§6.5): kNN selection at matched natural ratio still collapses paysim (0.897→0.033) and hurts banksim (0.901→0.785) but mildly helps eu_cc (0.834→0.845) [finding: results/runs_ablation]
...
- **H2 — starvation, adjudicated per dataset (2026-07-21): "not starvation" holds on paysim only.** The 6 paysim C3 runs split 2 fraud-rich vs 4 fraud-starved: the 2 fraud-rich runs — seed 7 tabpfn_3 PR-AUC 0.2978 at ~832 mean frauds/context (run `931d287e9c22`) and tabiclv2 0.2110 at ~820 (run `ee59299a0dcb`) — still collapse relative to C1 (~0.91), landing at the KR composition floor (≈75 % of the collapse is composition; the 4 starved runs at 0.0225–0.0629, ~1 fraud/context, add the rest). The fraud-rich evidence is explicitly **n=2**. On eu_cc the split runs the other way: the starved seeds (6 frauds, G=2) crash to 0.09–0.51 while the fraud-adequate seed 42 (105–107 frauds, G=9) nearly recovers C1 (0.755/0.765) → eu_cc's collapse is ≈90 % depletion/starvation, ≈10 % composition. Never argue from the per-dataset *average* fraud count (bimodal distributions) [finding: results/runs, C3 context_fraud_n per seed; results/runs_ablation]
